...
export class ReceivePage {
    ...
    constructor(private platform: Platform, public navCtrl: NavController, private navParams: NavParams, private nem: NemProvider, private wallet: WalletProvider, private alert: AlertProvider, private toast: ToastProvider, private barcodeScanner: BarcodeScanner, private alertCtrl: AlertController, private loading: LoadingController, private keyboard: Keyboard, public translate: TranslateService, private clipboard: Clipboard) {
        this.amount = 0;
  ...
  /**
    ...
    /**
     * Copy the Full Address and Display it for user
     */

    public copyAddress() {
        if (this.platform.is('cordova')) {
            this.clipboard.copy(this.selectedWallet.address.plain()).then(_ => {
                this.toast.showCustomToast("Address Coppied to Clipboard", null);
                this.alert.showCustomAlert(this.selectedWallet.address.plain(),null)
            });
        } else {
