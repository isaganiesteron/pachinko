		...
		setInterval(() => {
			...
			this.storage.get('balance').then((val) => {
				if(val){
					...
					notificationText1 += "Welcome to Pachinko! This is your balance: ";
					notificationText2 += "Welcome to Pachinko! This is your balance: ";
				}
				...
				this.nem.getBalance(this.selectedWallet.address).then(bal => {
					...
					//let targetMosaicName = 'kidlet:kid'
					let targetMosaicName = 'pachinko:test'

					bal.forEach(function(curr,ind){
                    	var currCompare = curr.mosaicId.namespaceId+":"+curr.mosaicId.name
						if(currCompare == targetMosaicName)
					...
					if(this.balance.kid < updatedKidBal) {
						notificationText2 += "Pachinko: " + updatedKidBal + " (+" + (updatedKidBal - this.balance.kid).toFixed(2) + ")";
						kidChanged = true;
					...
					//		console.log(filePath);
							this.localNotifications.schedule({
								id: 1,
								title: 'Pachinko Balance Changed:',
