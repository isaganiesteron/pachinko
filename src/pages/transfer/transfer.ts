...
import {UpdateContactPage} from '../contact/update/update';
...
export class TransferPage {
    ...
    constructor(private platform: Platform, private contact: ContactProvider, public navCtrl: NavController, private navParams: NavParams, private nem: NemProvider, private wallet: WalletProvider, private alert: AlertProvider, private toast: ToastProvider, private barcodeScanner: BarcodeScanner, private alertCtrl: AlertController, private loading: LoadingController, private keyboard: Keyboard, public translate: TranslateService) {
        this.amount = 0;
        ...
        this.wallet.getSelectedWallet().then(
            value => {
                ...
                if (this.platform.is('cordova')) {
                    this.owner = this.selectedWallet.address.plain()
                    this._getContactsByOwner(this.owner);
                }
            }
        )
    }
    ...
    private _presentPrompt() {
        this.translate.get(['YOU_ARE_GOING_TO_SEND', 'AMOUNT', 'FEE', 'LEVY', 'CONFIRM_TRANSACTION', 'PASSWORD', 'CANCEL', 'CONFIRM', 'PLEASE_WAIT'], {}).subscribe((res) => {
            this._subtitleBuilder(res).then(subtitle => {
                ...
                let saveAddress = this.alertCtrl.create({
                    title: 'Would you like to save this address?',
                    message: 'Enter the name for this address\n'+this.rawRecipient,
                    inputs: [
                        {
                            name: 'Name',
                            placeholder: 'Name'
                        ...
                        },
                        {
                            ...
                            handler: data => {
                                this.navCtrl.push(UpdateContactPage,{
                                    'id': null,
                                    'owner': this.selectedWallet.address.plain(),
                                    'name': data.Name,
                                    ...
                                    if (this._canSendTransaction()) {
                                        this._confirmTransaction().subscribe(value => {
                                            ...
                                            this.toast.showTransactionConfirmed($ => {
                                                ...
                                                console.log(this.rawRecipient)
                                           
                                                this.contact.searchContactName(this.owner, this.rawRecipient).then(current => {
                                                    if(current.length < 1)
                                                        saveAddress.present();
                                                })
                                            });
    ...
    /**
     * Handles onChange of the user address book
     */
    public onSelectChange(selectedValue: any) {
        this.rawRecipient = selectedValue;
      }
}
